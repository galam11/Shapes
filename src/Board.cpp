...
namespace
{
    ...
    // Board cares if we are in the same row/column after rounding
    // as this is what matters when drawing
    bool sameRoundedCol(const Vertex& v1, const Vertex& v2)
    {
        return std::round(v1.m_col) == std::round(v2.m_col);
    }

    bool sameRoundedRow(const Vertex& v1, const Vertex& v2)
    {
...
void Board::drawLine(const Vertex& v1, const Vertex& v2)
{
    ...
    if (sameRoundedCol(v1, v2)) // parallel to the y axis
    {
    ...
    if (sameRoundedRow(v1, v2)) // parallel to the x axis
    {
